...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($captcha_input) || $user_answer_hash !== $captcha_hash_check) {
        $error = "‚ùå Incorrect Security Answer!";
    }
    ...
    elseif (!preg_match($userRegex, $username)) {
        $error = "‚ùå Invalid Username! Need 2 letters & 1 number.";
    }
    else {
        ...
        if (mysqli_num_rows($res_check) > 0) {
            ...
            if (mysqli_stmt_execute($stmt_ins)) {
                ...
                $subject = "‚úÖ Confirm Account - SCL Dormitory Booking";
                ...
                if(@mail($email, $subject, $body, $headers)) {
                    $success = "‚úÖ Registration Successful! Check your email to activate your account (valid for 15 minutes).";
                } else {
                    $success = "‚úÖ Registered! Email delivery failed. Contact Admin or login directly.";
                }
            } else {
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* Ì†ºÌºü Main Wrapper added for vertical centering */
        .main-wrapper {
            flex: 1; 
            display: flex;
        ...
        /* Ì†ºÌºü FADE IN EFFECT ADDED HERE */
        ...
        .register-container h2 { color: #224895; margin-bottom: 5px; font-size: 20px; margin-top: 0; white-space: nowrap; font-weight: bold; }
        ...
        .form-group input { width: 100%; padding: 8px; padding-right: 35px; border: 1px solid #ccc; border-radius: 5px; box-sizing: border-box; font-size: 15px; background: white; }

        .select2-container { width: 100% !important; max-width: 100%; }
        .select2-container .select2-selection--single { height: 38px !important; padding: 5px; border: 1px solid #ccc !important; border-radius: 5px !important; }
        .select2-container--default .select2-selection--single .select2-selection__arrow { top: 5px !important; }

        .toggle-password { position: absolute; right: 10px; top: 32px; cursor: pointer; color: #999; font-size: 15px; user-select: none; z-index: 2; }
        .toggle-password:hover { color: #2f6b96; }

        .success-icon { position: absolute; right: 10px; top: 32px; color: #28a745; font-size: 16px; display: none; z-index: 1; }
        .password-icon-adjust { right: 35px !important; }
        .select2-icon-adjust { top: 32px; right: 35px; z-index: 99; }

        .btn-register { width: 100%; padding: 10px; background: #2f6b96; color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer; margin-top: 5px; transition: all 0.3s; font-weight: bold;}
        .btn-register:hover { background: #1a4f75; transform: scale(1.02); }
        .btn-register:disabled { background-color: #cccccc !important; color: #666666; cursor: not-allowed; opacity: 0.6; transform: none; }

        .message-box { padding: 8px; border-radius: 5px; margin-bottom: 10px; font-size: 13px; text-align: center; }
        .error { background: #ffdddd; color: darkred; border-left: 5px solid red; font-weight: bold;}
        .success { background: #ddffdd; color: darkgreen; border-left: 5px solid green; font-weight: bold;}
        ...
        /* Ì†ºÌºü CUSTOM YELLOW FOOTER STYLE */
        .scl-footer {
            background: transparent !important;
            color: yellow !important; /* Yellow text as requested */
            font-weight: 600;
...
<body>
    ...
    <!-- Ì†ºÌºü Added Main Wrapper -->
    <div class="main-wrapper">
        <!-- Ì†ºÌºü Added fade-in class -->
        <div class="register-container fade-in">
            <img src="logo_login.png" alt="SCL Logo" class="logo_login-img">

            <h2>SCL ICT ASSET MANAGEMENT SYSTEM </h2>
    ...
    <!-- Ì†ºÌºü Added custom yellow footer since footer.php is disabled -->
    <div class="scl-footer">
        ¬© <?php echo date("Y"); ?> All Rights Reserved by <strong>SCL ICT Team</strong> | A Product of <strong> Sheltech Ceramics ICT Team.</strong>
    </div>
